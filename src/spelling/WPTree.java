...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		while(! wordsToExplore.isEmpty() && ! visited.contains(word2)){
			WPTreeNode curr = wordsToExplore.poll();
			List<String> neighbors = nw.distanceOne(curr.getWord(), true);
			
			for(String s: neighbors){
				if(!visited.contains(s)){
					WPTreeNode n = new WPTreeNode(s, curr);
					curr.addChild(s);
					visited.add(s);
					wordsToExplore.add(n);
					if(n.getWord().equals(word2)){
